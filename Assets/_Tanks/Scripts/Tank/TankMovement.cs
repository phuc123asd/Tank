...
namespace Tanks.Complete
{
    ...
    public class TankMovement : MonoBehaviour
    {
        ...
        [Tooltip("How quickly the tank reaches its target speed.")]
        public float m_Acceleration = 18f;
        [Tooltip("How quickly the tank slows down when input is released.")]
        public float m_Deceleration = 24f;
        [Tooltip("Ignore tiny input values to avoid drifting.")]
        public float m_InputDeadZone = 0.1f;
        ...
        private float m_CurrentSpeedInput;          // Smoothed movement input used by the physics step.
        ...
        private bool m_UseCameraRelativeControl;
        ...
        private void Start ()
        {
            ...
            m_UseCameraRelativeControl = m_IsDirectControl || IsUsingGamepadControl();
        ...
        private void Update ()
        {
            ...
            if (!m_IsComputerControlled)
            {
                m_MovementInputValue = ApplyDeadZone(m_MoveAction.ReadValue<float>());
                m_TurnInputValue = ApplyDeadZone(m_TurnAction.ReadValue<float>());
            }
        ...
        private void UpdateRequestedDirection()
        {
            ...
            {
                m_RequestedDirection = Vector3.zero;
                return;
            }
            ...
            var camForward = m_CameraTransform.forward;
                ...
                {
                    camForward = m_CameraTransform.up;
                    camForward.y = 0;
                }
            ...
            m_RequestedDirection = Vector3.ClampMagnitude(m_RequestedDirection, 1f);

            if (m_RequestedDirection.magnitude < m_InputDeadZone)
            {
                m_RequestedDirection = Vector3.zero;
            }
        }
        ...
        private void Move (float fixedDeltaTime)
        {
            ...
            if (m_UseCameraRelativeControl)
            {
                ...
                if (speedInput > 0f)
                {
                    speedInput *= 1.0f - Mathf.Clamp01((Vector3.Angle(m_RequestedDirection, transform.forward) - 90f) / 90f);
                }
            }
            else
            {
            ...
            float speedChange = Mathf.Abs(speedInput) > Mathf.Abs(m_CurrentSpeedInput) ? m_Acceleration : m_Deceleration;
            m_CurrentSpeedInput = Mathf.MoveTowards(m_CurrentSpeedInput, speedInput, speedChange * fixedDeltaTime);
            
            // Create a vector in the direction the tank is facing with a magnitude based on the input, speed and the time between frames.
            Vector3 movement = transform.forward * m_CurrentSpeedInput * m_Speed;

            // Apply this movement to the rigidbody's position.
            m_Rigidbody.linearVelocity = movement + m_ExplosionForceValue;
            m_ExplosionForceValue = Vector3.Lerp(m_ExplosionForceValue, Vector3.zero, fixedDeltaTime * 3f); // 3f = braking speed
        }
        ...
        private void Turn (float fixedDeltaTime)
        {
            ...
            if (m_UseCameraRelativeControl)
            {
                ...
                float angleTowardTarget = Vector3.SignedAngle(transform.forward, m_RequestedDirection, Vector3.up);
                var rotatingAngle = Mathf.Sign(angleTowardTarget) * Mathf.Min(Mathf.Abs(angleTowardTarget), m_TurnSpeed * fixedDeltaTime);
                turnRotation = Quaternion.AngleAxis(rotatingAngle, Vector3.up);
            }
            else
            {
                float turn = m_TurnInputValue * m_TurnSpeed * fixedDeltaTime;
        ...
        private bool IsUsingGamepadControl()
        {
            return m_InputUser != null
                && m_InputUser.InputUser.valid
                && m_InputUser.InputUser.controlScheme.HasValue
                && m_InputUser.InputUser.controlScheme.Value.name == "Gamepad";
        }

        private float ApplyDeadZone(float value)
        {
            return Mathf.Abs(value) < m_InputDeadZone ? 0f : Mathf.Clamp(value, -1f, 1f);
        }
